...
import {
  type WailsCallError,
  type WailsEventPayload,
  type WailsMessage,
  type WailsResponse,
  readRuntimeFlags,
} from "./types.js";

/** 可被取消的 Promise：携带 callId 与 cancel()。 */
export interface CancellablePromise<T> extends Promise<T> {
  /** 本次调用的 ID（取消用）。 */
  callId: string;
  /** 取消本次调用（向后端发送 cancel 并标记 pending 为已取消）。 */
  cancel: () => void;
}
...
type EventCallback = (data: unknown, senderWindowId?: number | null) => void;

const CHUNK_THRESHOLD = 512 * 1024; // 512KB：原生/HTTP 分界（按 JSON 字符串长度）
const CHUNK_SIZE = 1_000_000; // 1MB：单 chunk 字节上限（后端限制 ≤ 1MB）
const DEFAULT_WS_URL = "ws://localhost:34116/wails/ws";

function randomId(len = 21): string {
...
export class Transport {
  ...
  private counter = 0;
  private nativePost: ((s: string) => void) | undefined;
  private ws: WebSocket | undefined;
  private wsUrl = DEFAULT_WS_URL;
  private messageUrl = "/wails/message";
  ...
  init(): void {
    ...
    if (this.serverMode) this.connectWs();
  }
  ...
  private detectNativePost(): ((s: string) => void) | undefined {
  ...
  private registerDownlink(): void {
    const w = globalThis as unknown as {
      ...
      __wailsNative?: { onMessage?: (json: string) => void };
      _wailsEmitEvent?: (name: string, data: unknown, senderWindowId?: number | null) => void;
    };
    const onMsg = (e: { data: unknown }) => {
      if (typeof e.data === "string") this.onNativeMessage(e.data);
    };
    ...
    // 供原生 / Android 桥接调用
    w.__wailsNative = { onMessage: (json: string) => this.onNativeMessage(json) };
    ...
    w._wailsEmitEvent = (name: string, data: unknown, senderWindowId?: number | null) =>
      this._emitEvent(name, data, senderWindowId ?? null);
  }
  ...
  private connectWs(): void {
    ...
    this.ws.onmessage = (e: MessageEvent) => {
      try {
        ...
        this.handleResponse(data as WailsResponse);
      } catch {
        /* ignore malformed */
      }
    };
  ...
  invoke<T>(type: string, payload: unknown): CancellablePromise<T> {
    this.ensureInit();
    const id = this.nextId();
    const promise = new Promise<T>((resolve, reject) => {
      this.pending.set(id, { resolve: resolve as (v: unknown) => void, reject, cancelled: false });
  ...
  once(name: string, cb: EventCallback): () => void {
  ...
  emit<D>(name: string, data: D): void {
    const payload: WailsEventPayload<D> = { name, data };
    this.send("event.emit", payload);
  }

  private _emitEvent(name: string, data: unknown, senderWindowId: number | null = null): void {
  ...
  private async dispatch(msg: WailsMessage): Promise<void> {
    if (this.serverMode && this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify(msg));
      return;
    }
  ...
  private async sendHttp(msg: WailsMessage): Promise<void> {
  ...
  private async sendCancelHttp(msg: WailsMessage): Promise<void> {
    try {
  ...
  private async handleHttpResponse(resp: Response): Promise<void> {
    try {
      const text = await resp.text();
      if (!text) return;
      const data = JSON.parse(text) as WailsResponse;
  ...
  private onNativeMessage(json: string): void {
    try {
      const data = JSON.parse(json) as WailsResponse | { type: "event"; name: string; data: unknown };
      if (data && (data as { type: string }).type === "event" && typeof (data as { name: string }).name === "string") {
        this._emitEvent((data as { name: string }).name, (data as { data: unknown }).data);
        return;
      }
      this.handleResponse(data as WailsResponse);
    } catch {
  ...
  private handleResponse(data: WailsResponse): void {
    const id = data?.id;
    if (!id) return;
    const p = this.pending.get(id);
    if (!p) return;
    this.pending.delete(id);
    if (p.cancelled) return; // 已取消，丢弃
    try {
 ...
 */
export function unpack(data: WailsResponse): unknown {
  if (!data) throw toCallError("空响应");
  const outer = data.result;
  if (outer && typeof outer === "object" && "error" in outer) {
    if (outer.error) throw toCallError(outer.error as WailsCallError | string);
    return outer.result;
  }
  return outer;
}
...
export function bindingId(fullName: string): number {
