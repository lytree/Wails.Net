...
import type { CancellablePromise } from "../internal/transport.js";

/** 参数打包模式，对应线协议约定。 */
export type PackMode =
  /** 无参数 → 发送 `[]`（后端传 default）。 */
  | "none"
  /** 单业务参数 → 包装为 `[{...}]`（后端按唯一参数整体反序列化）。 */
  | "single"
  /** 多位置参数 → 原样展开 `[...]`（后端按位置逐个反序列化）。 */
  | "spread";
 ...
 * 独立为纯函数，便于单测与复用。
 */
function packArgs(pack: PackMode, args: unknown[]): unknown[] {
  switch (pack) {
    case "none":
      return [];
    case "single":
...
export function defineCommand<A extends unknown[], R>(
  ...
  return (...args: unknown[]) => call<R>(name, packArgs(pack, args));
}
