...
Wails v3（Go 实现，参考版本 `v3.0.0-alpha.102`）需要在 .NET 平台上重新实现，目标平台为 Windows 与 Linux（macOS/iOS/Android 暂不实现）。这是一次跨语言移植，不仅仅是语法转换，更需要在以下约束下做出整套技术决策：
...
8. **Avalonia / MAUI 作为跨平台 GUI**：放弃。Wails.Net 的 GUI 由 WebView 承载，仅需原生窗口壳与系统 API，引入完整 GUI 框架属于过度设计。
