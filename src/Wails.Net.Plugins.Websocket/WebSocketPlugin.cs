...
namespace Wails.Net.Plugins.WebSocket;
    ...
    private static async Task ReceiveLoopAsync(string connectionId, ClientWebSocket ws)
    {
        ...
        try
        {
            while (ws.State == WebSocketState.Open)
            {
                ...
                WailsApplication.Get()?.Events.Emit("websocket:message", data, null);
            }
        }
        catch
        {
        ...
        finally
        {
            ...
            WailsApplication.Get()?.Events.Emit("websocket:closed", connectionId, null);
        }
    }
}
