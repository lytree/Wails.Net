...
// 类型别名：MenuPlugin 声明于 namespace Wails.Net.Plugins.Menu 中，
// 简单名 "Menu" 会被解析为同名命名空间（CS0118），故用别名引用基座菜单类型。
using MenuType = Wails.Net.Application.Menus.Menu;
...
namespace Wails.Net.Plugins.Menu;
...
public class MenuPlugin : IPlugin
{
    ...
    public void Configure(IPluginContext context)
    {
        ...
        commands.MapCommand("menu.getApplicationMenu", (Func<ICommandContext, MenuType?>)(ctx =>
    ...
    private static MenuType ResolveParentOrThrow(ICommandContext ctx, string parentId)
    {
    ...
    /// <returns>匹配的菜单项，未找到返回 null。</returns>
    private static MenuItem? FindMenuItemInMenu(MenuType menu, uint targetId)
    {
