...
namespace Wails.Net.Plugins.PowerManagement;
...
public class PowerManagementPlugin : IPlugin, IDisposable
{
    ...
    {
        Suspend?.Invoke();
        WailsApplication.Get()?.HandlePlatformEvent((uint)ApplicationEventType.Suspend);
    }
    ...
    {
        Resume?.Invoke();
        WailsApplication.Get()?.HandlePlatformEvent((uint)ApplicationEventType.Resume);
    }
