...
/// <summary>
/// 处理 <c>/wails/transport.js</c> 路由的中间件。
/// 对应 Wails v3 Go 版本中为前端提供传输层 JS 代码的路由处理逻辑。
/// </summary>
public class TransportJsRoute : IMiddleware
{
    /// <summary>
    /// 路由匹配的路径常量。
    /// </summary>
    public const string RoutePath = "/wails/transport.js";

    /// <summary>
    /// 异步处理请求：当路径匹配 <c>/wails/transport.js</c> 时返回传输层 JS 代码，
    /// 否则将请求转发给链中下一个处理器。
    /// </summary>
    /// <param name="path">请求的资源路径。</param>
    /// <param name="next">链中下一个处理器的委托。</param>
    /// <returns>传输层 JS 代码字节组；若路径不匹配则返回下一处理器的结果。</returns>
    public Task<byte[]?> ProcessAsync(string path, Func<string, Task<byte[]?>> next)
    {
        ...
        {
            // 占位实现：实际传输层代码将在后续阶段注入。
            var content = "// Wails.NET transport.js placeholder\n";
